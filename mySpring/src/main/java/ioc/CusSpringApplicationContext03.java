 ...
 */
//开始实现第五阶段 加上了后置处理器
public class CusSpringApplicationContext03 {
    ...
    public CusSpringApplicationContext03(Class config) {
        this.config = config;
        //扫描
...
//            如果是多例 不创建对象
    ...
    public void beanDefinitionMapScan(Class config) {
        ...
        URL resource = CusSpringApplicationContext03.class.getClassLoader().getResource(path);
...
//                这一步可以拿到set好的bean 开始进行前置处理 如果加了业务逻辑 其实反不返回没啥意义 本来就是引用对象
                Object current = beanPostProcessor.postProcessBeforeInitialization(instance, instance.getClass().getName());
//                这里属于优化 不想优化拉倒
                if (current != null) {
                    instance = current;
                }
...
//                这一步可以拿到init好的bean 开始进行后置处理 如果加了业务逻辑  就把新的bean返回回来
                Object current = beanPostProcessor.postProcessAfterInitialization(instance, instance.getClass().getName());
                //                这里属于优化 不想优化拉倒
                if (current != null) {
                    instance = current;
                }
